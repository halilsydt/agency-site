...
/**
 * Path data for the Atlas "arrow + S" brand mark.
 * Copied verbatim from `docs/design system/atlas/assets/logo-mark.svg`
 * (viewBox "31 28 353 511"). `S_PATH` is the stylised "S" body; `ARROW_PATH`
 * is the upward arrow. Do not hand-edit — these are the canonical artwork bounds.
 */
const S_PATH =
...
/** Atlas colour pairs for the mark — `[arrow, s]`. */
const COLORS = {
  /** On light (paper) — header. */
  light: { arrow: "#0E8C5A", s: "#15201B" },
  /** On ink (dark) — footer. */
  dark: { arrow: "#6FD3A4", s: "#FAF9F5" },
} as const;

/**
 * Props for the {@link LogoMark} component.
 */
export interface LogoMarkProps {
  /**
   * Renders the on-dark variant (mint arrow on off-white S) for use on the ink
   * footer. Defaults to the on-light header variant.
   */
  onDark?: boolean;
  /** Optional className forwarded to the `<svg>` (controls sizing). */
  className?: string;
}

/**
 * Scalenty "arrow + S" brand mark, recoloured to the Atlas palette.
 * Used by both the header (on light) and footer (on dark).
 *
 * @example
 * ```tsx
 * <LogoMark className="h-6 w-auto" />          // header
 * <LogoMark onDark className="h-6 w-auto" />   // footer
 * ```
 */
export function LogoMark({ onDark = false, className }: LogoMarkProps): React.ReactElement {
  const { arrow, s } = onDark ? COLORS.dark : COLORS.light;
  return (
    <svg viewBox="31 28 353 511" fill="none" aria-label="Scalenty" role="img" className={className}>
      <path fill={s} d={S_PATH} />
      <path fill={arrow} d={ARROW_PATH} />
    </svg>
  );
}
